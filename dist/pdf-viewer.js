...
function Er({ wasmUrl: e, standardFontDataUrl: t } = {}) {
	return {
		wasmUrl: e ? Or(e) : Dr(ie, Sr),
		standardFontDataUrl: t ? Or(t) : Dr(N, Cr)
	};
}
function Dr(e, t) {
	return wr(Or(e), t);
...
function kr({ workerSrc: e, workerPort: t } = {}) {
	if (t) {
...
function Vr(e, { workerSrc: t, workerPort: n, wasmUrl: i, standardFontDataUrl: o, cacheSize: u = 3, onLoadError: d } = {}) {
	let [f, p] = c(Br, zr), [m, h] = c((e) => e + 1, 0), g = F(d), _ = e ? Nr(e) : null, v = _ === null ? null : f.documents.get(_), y = _ !== null && f.failure?.key === _ ? f.failure.error : null, b = F(f.documents), x = l(null);
	a(() => {
		if (n) return;
		kr({
			workerSrc: t,
		...
		for (let e of b.current.values()) Lr(e.pdfDoc);
	}, [b]);
	let S = r(() => {
		_ !== null && p({
			type: Rr.EVICTED,
	...
	a(() => {
		if (f.evicted?.length) for (let e of f.evicted) Lr(e.pdfDoc);
	}, [f.evicted]), a(() => {
		if (!e || _ === null) return;
		...
		return (async () => {
			...
			try {
				...
				e.message = Ar(e), console.error("[@armsolusi/pdf-viewer] Failed to load document:", e), p({
					type: Rr.FAILED,
					key: _,
					error: e
				}), g.current?.(e);
...
async function Hr(e, t) {
	let n = [];
	for (let r = 1; r <= e.numPages; r += Pr) {
		let i = Math.min(r + Pr - 1, e.numPages), a = [];
...
function Ur({ pageCount: e, container: t, containerRef: n }) {
...
var Wr = 24, Gr = (e, t) => Math.hypot(e.x - t.x, e.y - t.y), Kr = (e, t) => ({
	x: (e.x + t.x) / 2,
	y: (e.y + t.y) / 2
});
function qr({ container: e, containerRef: t, setScale: n, anchorAtPointer: r, cancelStrokeRef: i, isDraggingRef: o }) {
	...
	a(() => {
		...
		let r = u.current, a = () => {
			...
			e.preventDefault();
			let [n, i] = [...r.values()], a = Gr(n, i);
			if (a < Wr) return;
			let o = Kr(n, i), l = document.elementFromPoint(o.x, o.y);
		...
		let e = (e) => {
			...
			if (e.ctrlKey || e.metaKey) switch (e.key) {
				...
				case "d":
					...
					return;
				case "c":
					if (Jr(e.target) || !window.getSelection()?.isCollapsed) return;
					n.onCopy?.();
					return;
				case "v":
					if (Jr(e.target)) return;
			...
			if (e.key === "Delete" || e.key === "Backspace") {
				if (Jr(e.target) || !n.onDelete?.()) return;
				e.preventDefault();
				return;
			}
...
var Xr = "specimen", Zr = Object.freeze({
	SPECIMEN: "specimen",
	STAMP: "stamp"
}), Qr = Object.freeze({
	CONFIG: "config",
...
function $r({ specimenAsset: e, stampAssets: t }) {
	let [n, i] = u({}), o = l([]);
	a(() => {
		let e = o.current;
		return () => {
	...
	return {
		...
		list: s(() => Object.entries(d).map(([e, t]) => ({
			id: e,
			kind: t.kind ?? Zr.STAMP,
			source: t.source ?? Qr.CONFIG,
			label: t.label ?? e,
			src: t.src
		})), [d]),
		addUploadedAsset: f
...
function ei(e, t) {
	let n = {}, r = (e, t) => {
		!e || !t?.src || (n[e] = {
			...t,
			id: e,
			kind: t.kind === Zr.SPECIMEN ? Zr.SPECIMEN : Zr.STAMP,
			source: Qr.CONFIG,
			label: t.label ?? e
		});
	};
...
function ti({ annotations: e = [], assets: t = {} } = {}) {
	let n = {
		specimen: 0,
		stamp: 0,
		image: 0,
	...
	for (let r of e) switch (r?.type) {
		case W.IMAGE:
			n.image += 1, n.total += 1, t[r.assetId]?.kind === Zr.SPECIMEN ? n.specimen += 1 : n.stamp += 1;
			break;
		case W.TEXT:
			n.text += 1, n.total += 1;
			break;
...
function oi(e = ri) {
	...
	return {
		...
		setState(e) {
			if (!e || !Object.keys(e).some((n) => !ii(t[n], e[n]))) return !1;
			t = {
				...t,
				...e
			};
...
function pi({ src: e, documentId: t, config: n = {}, viewerRef: i, viewer: c }) {
	let { specimenAsset: d, stampAssets: f, onSpecimenChange: h, onAnnotationsChange: g, onAnnotationsSnapshot: _, onDownload: v, onLoadError: y, canDownload: b = !0, allowMultipleStamps: x = !0, maxStamps: S = null, rotateExportedPages: C = !0, toolbar: w, renderToolbar: T, workerSrc: E, workerPort: ee, wasmUrl: D, standardFontDataUrl: O, documentCacheSize: te } = n, [k, ne] = u(null), A = l(null), j = r((e) => {
		A.current = e, ne(e);
	}, []), [M, re] = u(!1), [N, ie] = u({}), { pdfDoc: P, pageSizes: I, sourceBytes: oe, status: L, error: R, reload: se, documentKey: ce, rememberScroll: le, getRememberedScroll: ue } = Vr(e, {
		workerSrc: E,
		workerPort: ee,
		wasmUrl: D,
		standardFontDataUrl: O,
		cacheSize: te,
		onLoadError: y
	}), de = P?.numPages ?? 0, { activePageIndex: fe, activePageRef: me, renderWindow: he, registerPage: ge, scrollToPage: _e } = Ur({
		pageCount: de,
		container: k,
		containerRef: A
	});
	...
	let H = tt(), { annotations: be, canUndo: xe, canRedo: Se } = et(), { assets: U, list: Ce, addUploadedAsset: W } = $r({
		specimenAsset: d,
		stampAssets: f
	}), { hasSpecimen: G, hasAnnotation: we, counts: K } = s(() => ti({
		annotations: Pe(be),
		assets: U
	}), [be, U]), q = s(() => (Ce.find((e) => e.kind === Zr.SPECIMEN) ?? Ce[0])?.id, [Ce]), De = s(() => ({
		pdfDoc: P,
		documentKey: ce,
		pageSizes: I,
		pageRotations: N,
	...
	a(() => {
		Me.current !== G && (Me.current = G, ke.current?.(G)), ni(Ne.current, K) || (Ne.current = K, Ae.current?.(K));
	}, [
		G,
		K,
		ke,
	...
	let Fe = r(async (e) => {
		...
		}) || fi(t, n.src), i.width && i.height && (r = ui * (i.height / i.width));
		let a = Te({
			assetId: t,
	...
	let Ge = s(() => Ce.filter((e) => e.source !== Qr.UPLOAD), [Ce]), Ke = s(() => Ce.filter((e) => e.source === Qr.UPLOAD), [Ce]), qe = s(() => ({
		...We,
		api: Y,
		configuredAssets: Ge,
		uploadedAssets: Ke,
...
var mi = t(function({ src: e, documentId: t, config: n, viewer: r }, i) {
	return /* @__PURE__ */ p(ut, {
		labels: n?.labels,
		children: /* @__PURE__ */ p(Qe, {
			documentId: t,
			initialAnnotations: n?.initialAnnotations,
			children: /* @__PURE__ */ p(it, { children: /* @__PURE__ */ p(pi, {
				src: e,
				documentId: t,
				config: n ?? {},
				viewerRef: i,
...
function gi(e, t = hi) {
	let n = r((t) => e?.subscribe(t) ?? _i, [e]), i = r(() => t(e?.getState() ?? ri), [e, t]);
	return d(n, i, i);
}
function _i() {}
//#endregion
//#region src/PDFViewer/utils/flattenPdf.js
async function vi({ src: e, annotations: t = [], stampAssets: n, specimenAsset: r, pageRotations: i = {}, rotateExportedPages: a = !0 } = {}) {
	if (!e) throw Error("flattenPdf: `src` is required");
	return Rn(await jr(e), t, ei(n, r), {
		pageRotations: i,
		rotateExportedPages: a
	});
}
//#endregion
export { W as ANNOTATION_TYPES, ot as DEFAULT_LABELS, qt as DEFAULT_TOOLBAR_ACTIONS, mi as PDFViewer, vi as flattenPdf, ci as usePdfViewer, gi as useViewerState };
