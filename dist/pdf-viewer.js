...
var or = !1, sr = "/.vite/deps/", cr = "/@armsolusi/pdf-viewer/dist/";
function lr(e) {
	let t = `${sr}pdf.worker.min.mjs`;
	return e?.includes(t) ? e.replace(t, `${cr}pdf.worker.min.mjs`) : e;
}
var ur = lr(N);
function dr({ workerSrc: e, workerPort: t } = {}) {
	if (t) {
	...
	if (!(O.GlobalWorkerOptions.workerSrc || O.GlobalWorkerOptions.workerPort)) {
		if (ur) {
			O.GlobalWorkerOptions.workerSrc = ur;
			return;
		}
...
function fr(e) {
	let t = e?.message ?? "";
	return /fake worker|worker/i.test(t) ? `${t}\n\nThe pdf.js worker shipped with @armsolusi/pdf-viewer could not be loaded. If you are on Vite, add:

  optimizeDeps: { exclude: ['@armsolusi/pdf-viewer'] }

to vite.config.js. Otherwise serve a copy yourself and pass config.workerSrc.` : t;
}
//#endregion
...
function gr(e, { workerSrc: t, workerPort: n, onLoadError: i } = {}) {
	let [o, c] = u(hr), [l, d] = u(0), f = P(i), p = r(() => d((e) => e + 1), []);
	a(() => {
		if (!e) return;
		dr({
			workerSrc: t,
		...
		return (async () => {
			try {
				let t = await pr(e);
				if (r || (i = O.getDocument({ data: mr(t) }), a = await i.promise, r)) return;
				let n = [];
				...
				e.message = fr(e), console.error("[@armsolusi/pdf-viewer] Failed to load document:", e), c({
					...hr,
					status: "error",
					error: e
				}), f.current?.(e);
...
function _r({ pageCount: e, container: t, containerRef: n }) {
...
var vr = 24, yr = (e, t) => Math.hypot(e.x - t.x, e.y - t.y), br = (e, t) => ({
	x: (e.x + t.x) / 2,
	y: (e.y + t.y) / 2
});
function xr({ container: e, containerRef: t, setScale: n, anchorAtPointer: r, cancelStrokeRef: i, isDraggingRef: o }) {
	...
	a(() => {
		...
		let r = u.current, a = () => {
			...
			e.preventDefault();
			let [n, i] = [...r.values()], a = yr(n, i);
			if (a < vr) return;
			let o = br(n, i), l = document.elementFromPoint(o.x, o.y);
		...
		let e = (e) => {
			...
			if (e.ctrlKey || e.metaKey) switch (e.key) {
				...
				case "d":
					...
					return;
				case "c":
					if (Sr(e.target) || !window.getSelection()?.isCollapsed) return;
					n.onCopy?.();
					return;
				case "v":
					if (Sr(e.target)) return;
			...
			if (e.key === "Delete" || e.key === "Backspace") {
				if (Sr(e.target) || !n.onDelete?.()) return;
				e.preventDefault();
				return;
			}
...
function Er({ specimenAsset: e, stampAssets: t }) {
	let [n, i] = u({}), o = l([]);
	a(() => {
		let e = o.current;
		return () => {
...
function Or({ annotations: e = [], assets: t = {} } = {}) {
	let n = {
		specimen: 0,
		stamp: 0,
		image: 0,
...
var Mr = (e) => typeof e == "object" && !!e && !Array.isArray(e);
function Nr(e = Ar) {
	...
	return {
		...
		setState(e) {
			if (!e || !Object.keys(e).some((n) => !jr(t[n], e[n]))) return !1;
			t = {
				...t,
				...e
			};
...
function Br({ src: e, config: t = {}, viewerRef: n, viewer: i }) {
	let { specimenAsset: c, stampAssets: d, onSpecimenChange: f, onAnnotationsChange: h, onDownload: g, onLoadError: _, canDownload: v = !0, allowMultipleStamps: y = !0, maxStamps: b = null, rotateExportedPages: x = !0, toolbar: S, renderToolbar: C, workerSrc: w, workerPort: T } = t, [E, ee] = u(null), D = l(null), O = r((e) => {
		D.current = e, ee(e);
	}, []), [te, ne] = u(!1), [k, A] = u({}), { pdfDoc: j, pageSizes: M, sourceBytes: re, status: N, error: ie, reload: ae } = gr(e, {
		workerSrc: w,
		workerPort: T,
		onLoadError: _
	}), oe = j?.numPages ?? 0, { activePageIndex: se, activePageRef: I, renderWindow: L, registerPage: ce, scrollToPage: le } = _r({
		pageCount: oe,
		container: E,
		containerRef: D
	}), R = lt({
	...
	let B = Ke(), { annotations: de, canUndo: fe, canRedo: me } = Ge(), { assets: H, list: he, addUploadedAsset: ge } = Er({
		specimenAsset: c,
		stampAssets: d
	}), { hasSpecimen: U, hasAnnotation: _e, counts: W } = s(() => Or({
		annotations: ke(de),
		assets: H
	...
	a(() => {
		we.current !== U && (we.current = U, G.current?.(U)), kr(Te.current, W) || (Te.current = W, Ce.current?.(W));
	}, [
		U,
		W,
		G,
	...
	let Ee = r(async (e) => {
		...
		}) || zr(t, n.src), i.width && i.height && (r = Lr * (i.height / i.width));
		let a = be({
			assetId: t,
			pageIndex: I.current,
			width: Lr,
	...
	let Le = s(() => he.filter((e) => e.source !== Tr.UPLOAD), [he]), Re = s(() => he.filter((e) => e.source === Tr.UPLOAD), [he]), ze = s(() => ({
		...Ie,
		api: q,
		configuredAssets: Le,
		uploadedAssets: Re,
...
var Vr = t(function({ src: e, config: t, viewer: n }, r) {
	return /* @__PURE__ */ p(tt, {
		labels: t?.labels,
		children: /* @__PURE__ */ p(Ue, { children: /* @__PURE__ */ p(Ye, { children: /* @__PURE__ */ p(Br, {
			src: e,
			config: t ?? {},
			viewerRef: r,
			viewer: n
...
function Ur(e, t = Hr) {
	let n = r((t) => e?.subscribe(t) ?? Wr, [e]), i = r(() => t(e?.getState() ?? Ar), [e, t]);
	return d(n, i, i);
}
function Wr() {}
//#endregion
export { W as ANNOTATION_TYPES, Ze as DEFAULT_LABELS, Lt as DEFAULT_TOOLBAR_ACTIONS, Vr as PDFViewer, Fr as usePdfViewer, Ur as useViewerState };
